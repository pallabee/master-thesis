...
class DiscreteDenoisingDiffusion(pl.LightningModule):
    def __init__(self, cfg, dataset_infos, train_metrics, sampling_metrics, visualization_tools, extra_features,
        ...
        if cfg.model.transition == 'uniform':
            ...
            #                                                        y_classes=self.ydim_output, f_classes=self.Fdim_output)
            # x_limit = torch.ones(self.Xdim_output) / self.Xdim_output
            # e_limit = torch.ones(self.Edim_output) / self.Edim_output
            # y_limit = torch.ones(self.ydim_output) / self.ydim_output
            # f_limit = torch.ones(self.Fdim_output) / self.Fdim_output
            # self.limit_dist = utils.PlaceHolder(X=x_limit, E=e_limit, y=y_limit, Feat=f_limit)
    ...
    def compute_val_loss(self, pred, noisy_data, X, E, y,node_mask, test=False):
       ...
       """
        ...
        print('x_loss', self.val_X_logp(X * prob0.X.log()))
        print('e_loss', self.val_E_logp(E * prob0.E.log()))
        ...
        if wandb.run:
            wandb.log({"kl prior": kl_prior.mean(),
                       ...
                       "x_loss": self.val_X_logp(X * prob0.X.log()),
                       "e_loss": self.val_E_logp(E * prob0.E.log()),
