...
class NonMolecularVisualization:
    ...
    def visualize_non_molecule(self, graph, pos, path, iterations=100, node_size=100, largest_component=False):
        ...
        plt.figure()
        # nx.draw(graph, pos, font_size=5, node_size=node_size, with_labels=True, node_color=U[:, 1],
        #         cmap=plt.cm.coolwarm, vmin=vmin, vmax=vmax, edge_color='grey')
        color_class_map = {0: 'blue', 1: 'red', 2: 'darkgreen', 3: 'orange'}
        nx.draw(graph, pos, font_size=7, node_size=node_size, with_labels=True,
                node_color=[color_class_map[node[1]['color_val']] for node in graph.nodes(data=True)],
                 font_color='white',
                edge_color='grey')
    ...
    def visualize(self, path: str, graphs: list, num_graphs_to_visualize: int, log='graph'):
        ...
        for i in range(num_graphs_to_visualize):
            ...
            file_path1 = os.path.join(path, 'graph_{}.gexf'.format(i))
            nx.write_gexf(graph, file_path1)
